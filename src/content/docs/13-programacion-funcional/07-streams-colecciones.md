...
> 🗺️ **Estás en:** 🎯 **U122 · Programación Funcional** → 07 · Del stream a la colección: toList, Collectors, groupingBy
...
Es el equivalente a la sentencia SQL `GROUP BY`: en vez de recorrer a mano y montar el `HashMap` de la U12 con `getOrDefault`, declaras la clave y Java agrupa. Por dentro está usando un `HashMap`, pero tú no lo ves: el `Collectors` lo monta por ti.
...
> 🕶️ **Don Tip:** `groupingBy` admite un segundo argumento, el "colector aguas abajo": `groupingBy(clave, Collectors.counting())` te da `Map<clave, Long>` con cuántos elementos hay en cada grupo. El contador de frecuencias de la U12 en una sola línea.
...
La **A**. `groupingBy(p -> p, Collectors.counting())` agrupa las palabras por sí mismas (cada palabra es su propia clave) y el colector aguas abajo `counting()` cuenta cuántas veces aparece cada una: `sol` aparece 3 veces, `mar` 2 y `luna` 1. Las claves del mapa son únicas (esa es la regla de oro de los mapas de la U12), así que la C es imposible: no puede haber dos entradas con la misma clave. La B sería el resultado sin `counting()` (grupos de un elemento) y la D confunde el total con el grupo.
...
📚 [Volver al índice de la unidad](/ApuntesProgramacion/13-programacion-funcional) · **Anterior:** [06 · Operaciones terminales: collect, reduce, count, forEach](/ApuntesProgramacion/13-programacion-funcional/06-streams-terminales) · **Siguiente:** [08 · Optional: el que acepta la ausencia](/ApuntesProgramacion/13-programacion-funcional/08-optional)
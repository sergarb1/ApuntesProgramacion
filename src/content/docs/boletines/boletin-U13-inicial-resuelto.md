...
Dos trucos nuevos: `Arrays.stream(nombres)` convierte el array (de la U05) en stream, y `map(String::length)` transforma cada `String` en su longitud, cambiando el tipo del flujo a `Stream<Integer>`. El `forEach` con `System.out::println` es la terminal que imprime cada elemento.

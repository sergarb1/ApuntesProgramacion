...
Constructor privado + todo `static` = clase utilitaria, como `Math`. `estaOrdenado` devuelve `false` en cuanto encuentra una pareja fuera de orden (`return` temprano). El `for...each` de la U04 se puede usar ya: recorre sin índice.

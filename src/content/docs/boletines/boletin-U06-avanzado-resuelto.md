...
Los números con menos de 4 cifras se completan con ceros porque extraemos con `% 10` y `/ 10` sobre un array de 4 posiciones: el 21 se convierte en `{0, 0, 2, 1}`. El algoritmo de Kaprekar termina siempre (máximo unas pocas iteraciones) y llega a 6174... **excepto los repdigits** (1111, 5555...): la primera resta da 0 y, si entraras en el `while (numero != 6174)`, te quedarías dando vueltas con 0 para siempre. Por eso el problema oficial de AceptaElReto pide que los repdigits impriman `8`, y lo detectamos antes con `esRepdigit`. La bombolla del punto 4, reutilizada dentro del propio Kaprekar: el código de la U06 resolviendo problemas reales.

...
Salida: `09:05`. No necesita setters porque es **inmutable**: una vez creada, su valor no cambia jamás (los `final` lo garantizan). Cambiar la hora es crear una `Hora` nueva. El constructor usa `throw` (viste la U04) para los valores imposibles.
...
Constructor privado + todo `static` = clase utilitaria, como `Math`. `estaOrdenado` devuelve `false` en cuanto encuentra una pareja fuera de orden (`return` temprano). El `for...each` de la U05 se puede usar ya: recorre sin índice.

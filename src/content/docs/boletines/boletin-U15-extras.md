...
La `LinkedList` como `Queue` es la protagonista: `poll()` saca al primero y `offer()` lo devuelve al final si falla. Los jugadores que aciertan salen para siempre, y el último que queda es el ganador. Estructura de datos (U11) al servicio del problema de turnos.

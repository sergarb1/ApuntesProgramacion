...
Fíjate: `Cat` no toca `name` directamente (es privado en `Pet`), usa `getName()`. Eso es el triángulo perfecto: **encapsulación** (privado + getter) + **herencia** (`extends`) + **sobrescritura** (`@Override`). La herencia la profundizas en la U10, pero aquí ya la ves en acción.
...
`Persona` encapsula su nombre: `private` + constructor + un método que sabe saludar. El `main` solo se encarga de leer y crear objetos. `linea.substring(4)` se salta los 4 primeros caracteres (`"Soy "`). El `sc.nextLine()` extra tras el `nextInt()` consume el Enter, el clásico del Scanner que viste en la U03.

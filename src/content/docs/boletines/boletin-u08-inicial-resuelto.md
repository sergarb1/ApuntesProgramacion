...
El método mezcla dos mundos: usa los atributos del objeto (`nombre`, `nota`) con la lógica de decisión (`if`/`else`) que aprendiste en la U04. Dos objetos, dos estados, una clase.
...
El constructor guarda `draft` y `crew` con `this`, y `isWorthIt()` aplica la fórmula: cada miembro de la tripulación reduce el calado útil en 1.5 pies. Si lo que queda supera 20, el barco merece la pena. Objeto + operaciones de la U03, resuelto.

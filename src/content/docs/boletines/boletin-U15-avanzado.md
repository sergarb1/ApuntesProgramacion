...
**Pista:** carga el archivo con `props.load(Files.newInputStream(Path.of("db.properties")))` y usa `props.getProperty("url")`. `load` lanza una `IOException` (la viste en la U123 con los ficheros) además de la `SQLException`.

...
Eixida: `09:05`. No necessita setters perquè és **immutable**: una vegada creada, el seu valor no canvia mai (els `final` ho garanteixen). Canviar l'hora és crear una `Hora` nova. El constructor usa `throw` (vist en la U04) per als valors impossibles.
...
Constructor privat + tot `static` = classe utilitària, com `Math`. `estaOrdenat` retorna `false` en seguint que trobe una parella fora d'orde (`return` primerenc). El `for...each` de la U05 es pot usar ja: recorre sense índex.

...
La `LinkedList` com a `Queue` és la protagonista: `poll()` trau el primer i `offer()` el torna al final si falla. Els jugadors que encerten ixen per sempre, i l'últim que queda és el guanyador. Estructura de dades (U11) al servei del problema de torns.

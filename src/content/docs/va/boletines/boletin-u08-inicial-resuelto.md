...
El mètode mescla dos mons: usa els atributs de l'objecte (`nom`, `nota`) amb la lògica de decisió (`if`/`else`) que vas aprendre en la U04. Dos objectes, dos estats, una classe.
...
El constructor guarda `draft` i `crew` amb `this`, i `isWorthIt()` aplica la fórmula: cada membre de la tripulació reduïx el calat útil en 1.5 peus. Si el que queda supera 20, el vaixell val la pena. Objecte + operacions de la U03, resolt.

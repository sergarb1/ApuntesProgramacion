...
> 🗺️ **Estàs en:** 🗄️ **U124 · Connexió a BD amb JDBC** → 02 · Connexió a SQLite
...
I el **bonus no opcional**: tancar-ho tot. En Java modern, `try-with-resources` ho fa sol (ho vas vore a la U123 amb els fitxers; ací és exactament la mateixa idea). Un recurs que no es tanca és una fuga: en un servidor, cada connexió oberta consumeix memòria i hi ha un límit. Arriba el següent `getConnection()` i... "Too many connections".
...
`DriverManager.getConnection(...)` llança **`SQLException`**, una excepció *checked*: el compilador t'obliga a capturar-la o declarar-la. Vas aprendre les excepcions a la U04; ací conviuràs amb esta a diari. El correcte no és empassar-te-la en silenci, sinó mostrar què ha passat:
...
📚 [Tornar a l'índex de la unitat](/ApuntesProgramacion/va/15-conexion-bases-datos) · **Anterior:** [01 · Què és JDBC](/ApuntesProgramacion/va/15-conexion-bases-datos/01-que-es-jdbc) · **Següent:** [03 · Statement i ResultSet: consultar dades](/ApuntesProgramacion/va/15-conexion-bases-datos/03-statement-resultset)
...
En la U11 vas vore les col·leccions: llistes, conjunts, iteradors i la classe `Collections`. Però en declarar `ArrayList<String>` et va quedar una pregunta en l'aire: què és eixe `<String>`? I per què no puc ficar un `Integer` en una llista de Strings?
...
|---|---|---|
| [01 · Genèrics: el `<T>` que ho va canviar tot](/ApuntesProgramacion/va/12-genericos-mapas/01-que-son-genericos) | El lio abans dels genèrics, el compilador com a amic i `ArrayList<T>` | Tots |
| [02 · La teua pròpia classe genèrica: `Caja<T>`](/ApuntesProgramacion/va/12-genericos-mapas/02-clases-genericas) | Paràmetres de tipus, les convencions T/E/K/V/N i la teua primera classe amb buits | Tots |
| [03 · El diamant `<>` i type erasure](/ApuntesProgramacion/va/12-genericos-mapas/03-diamante-type-erasure) | `new Caja<>()`, la inferència i el mag que borra els tipus en el bytecode | Tots |
| [04 · Mètodes genèrics: funcions per a tot tipus](/ApuntesProgramacion/va/12-genericos-mapas/04-metodos-genericos) | El `<T>` abans del tipus de retorn, la inferència i els límits amb `Comparable` | Tots |
| [05 · Wildcards: `? extends T` i `? super T`](/ApuntesProgramacion/va/12-genericos-mapas/05-wildcards) | Covariància, contravariància i el principi PECS | Tots |
| [06 · HashMap: la guia telefònica](/ApuntesProgramacion/va/12-genericos-mapas/06-hashmap) | `put`, `get`, recórrer amb `keySet`/`values`/`entrySet` i comptar freqüències | Tots |
| [07 · TreeMap, LinkedHashMap i altres mapes](/ApuntesProgramacion/va/12-genericos-mapas/07-treemap-otros-mapas) | Ordre natural, ordre d'inserció, `firstKey`, `headMap`... | Tots |
| [08 · Map vs List vs Set: tria la teua estructura](/ApuntesProgramacion/va/12-genericos-mapas/08-elegir-estructura-datos) | Les tres preguntes que decideixen abans d'escriure la primera línia | Tots |
| [09 · Repàs interactiu](/ApuntesProgramacion/va/12-genericos-mapas/09-repaso-interactivo) | Sé el Código, Fireside, Laboratori, Crucigrama i més | Tots |
...
> 📌 La **RA6** es completa amb la **U05** (arrays, CE a), la **U11** (col·leccions, CEs c, d i e) i la **U123** (regex i XML, CEs g, h i i). Esta unitat cobreix els genèrics (CE f) i els mapes del paquet `java.util`.

---

## 🚪 Per on comence?

- Vens de la U11 i vols l'essencial? → Comença en el [punt 1](/ApuntesProgramacion/va/12-genericos-mapas/01-que-son-genericos) i el [punt 2](/ApuntesProgramacion/va/12-genericos-mapas/02-clases-genericas): els genèrics de tots els dies.
- Ja uses genèrics i vols mapes? → Ves directe al [punt 6](/ApuntesProgramacion/va/12-genericos-mapas/06-hashmap) i al [punt 7](/ApuntesProgramacion/va/12-genericos-mapas/07-treemap-otros-mapas): la guia telefònica.
- Només vens a pels trucs? → Salta al [punt 3](/ApuntesProgramacion/va/12-genericos-mapas/03-diamante-type-erasure) (el diamant i el type erasure) i al [punt 5](/ApuntesProgramacion/va/12-genericos-mapas/05-wildcards) (els wildcards).
- Vens a repassar? → Fes el [Repàs interactiu](/ApuntesProgramacion/va/12-genericos-mapas/09-repaso-interactivo) i després els [butlletins](/ApuntesProgramacion/va/boletines/boletin-u122-inicial).

**📍 Primer punt:** [01 · Genèrics: el `<T>` que ho va canviar tot](/ApuntesProgramacion/va/12-genericos-mapas/01-que-son-genericos)  
**⏭️ En acabar la unitat, continua en [U122 · Programació Funcional](/ApuntesProgramacion/va/13-programacion-funcional).**
...
> 🗺️ **Estàs en:** 🗺️ **U12 · Genèrics i Mapes** → 06 · HashMap: la guia telefònica
...
> 💡 **Consell:** per a recórrer, declara el mapa com `Map<String, Integer>` i usa `Map.Entry<K, V>` en el bucle: així, si demà canvies a `TreeMap`, no toques res. Programa contra la interfície, com en la U11.
...
La **C**. En "banana", "b-a-n-a-n-a": b=1, a=3, n=2. Així que `frec.get('a')` = 3 i `frec.get('n')` = 2 → **3 2**. El truc de `getOrDefault(c, 0) + 1` és l'heroi: si la lletra no estava, torna 0 i comença en 1; si estava, suma un al comptador. Este patró és EL patró dels mapes: el veuràs en la U123 amb fitxers i en mitja carrera de programació.
...
📚 [Tornar a l'índex de la unitat](/ApuntesProgramacion/va/12-genericos-mapas) · **Anterior:** [05 · Wildcards: `? extends T` i `? super T`](/ApuntesProgramacion/va/12-genericos-mapas/05-wildcards) · **Següent:** [07 · TreeMap, LinkedHashMap i altres mapes](/ApuntesProgramacion/va/12-genericos-mapas/07-treemap-otros-mapas)
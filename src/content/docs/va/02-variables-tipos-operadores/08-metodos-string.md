...
> 💡 **Detall pràctic:** `length()` és un **mètode** (amb parèntesis). És l'error clàssic del novat escriure `texto.length` sense parèntesis i que no compile. En canvi, per a un array (la U09) s'usa `.length` sense parèntesis. Els `String` porten parèntesis; els arrays, no.
...
1. **Porta parèntesis**: `length()` és un mètode de la classe `String`. (Els arrays usen `.length` sense parèntesis, però això és la U09.)
...
📚 [Tornar a l'índex de la unitat](/ApuntesProgramacion/va/02-variables-tipos-operadores) · **Anterior:** [07 · Math.random() i nombres aleatoris](/ApuntesProgramacion/va/02-variables-tipos-operadores/07-math-random-aleatorios) · **Següent:** [09 · Repàs interactiu](/ApuntesProgramacion/va/02-variables-tipos-operadores/09-repaso-interactivo)
...
> 🗺️ **Estàs en:** 🚀 **U02 · Introducció a Java i l'entorn de treball** → 04 · El depurador
...
El bucle `for` el veuràs a fons en la U04, però ho pots intuir: repeteix la línia de dins mentre `i` siga menor que 10, amb `i` valent 0, 1, 2... La línia clau és `sospechoso += i` (que és el mateix que `sospechoso = sospechoso + i`).

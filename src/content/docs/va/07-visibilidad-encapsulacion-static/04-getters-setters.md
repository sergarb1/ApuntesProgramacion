...
> 🗺️ **Estàs en:** 🔒 **U07 · Visibilitat, Encapsulació i Static** → 04 · Getters i setters
...
> 📝 **Nota:** `this` (el punt 5 de la U06) resol el lío que paràmetre i atribut es diguen igual. `this.nombre` és l'atribut; `nombre` a seques és el paràmetre. Sense `this`, t'assignaries el paràmetre a si mateix i l'atribut es quedaria amb el seu valor per defecte.
...
Pots, i funcionarien. Però la convenció `getX()` / `setX()` és la que esperen els frameworks (i, de pas, els examinadors). Quan en la U13 veges com es serialitzen objectes a JSON, entendràs per què dir-se de la manera estàndard estalvia maldecaps.
...
📚 [Tornar a l'índex de la unitat](/ApuntesProgramacion/va/07-visibilidad-encapsulacion-static) · **Anterior:** [03 · Encapsulació: el pilar de la POO](/ApuntesProgramacion/va/07-visibilidad-encapsulacion-static/03-encapsulacion) · **Següent:** [05 · Atributs static](/ApuntesProgramacion/va/07-visibilidad-encapsulacion-static/05-atributos-static)
...
> 🗺️ **Estàs en:** 📁 **U123 · Fitxers i Expressions Regulars** → 04 · PrintWriter i Scanner + File
...
En la U03 vas vore `System.out.printf` i `Scanner` pel teclat (punts 6 i 7). Ara tots dos tornen, però apuntant a **fitxers** en comptes de a la consola. `PrintWriter` escriu amb format; `Scanner` + `File` llig fitxers com si estigueres llegint del teclat, amb el seu `hasNextLine`, `nextInt` i companyia.
...
El `%n` és el salt de línia portable (no uses `\n` en `printf`, que en Windows no encerta). Si necessites repassar els formats de `printf` (`%d`, `%f`, `%s`, `%-10s`...), torna a la U03, punt 7.
...
📁 [Tornar a l'índex de la unitat](/ApuntesProgramacion/va/14-ficheros-regex) · **Anterior:** [03 · try-with-resources: tancar sense plorar](/ApuntesProgramacion/va/14-ficheros-regex/03-try-with-resources) · **Següent:** [05 · NIO: Files i Paths (la forma moderna)](/ApuntesProgramacion/va/14-ficheros-regex/05-nio-files-paths)
...
> 💡 **Detalle práctico:** `length()` es un **método** (con paréntesis). Es el error clásico del novato escribir `texto.length` sin paréntesis y que no compile. En cambio, para un array (la U04) se usa `.length` sin paréntesis. Los `String` llevan paréntesis; los arrays, no.
...
1. **Lleva paréntesis**: `length()` es un método de la clase `String`. (Los arrays usan `.length` sin paréntesis, pero eso es la U04.)
...
📚 [Volver al índice de la unidad](/ApuntesProgramacion/02-variables-tipos-operadores) · **Anterior:** [08 · Math.random() y números aleatorios](/ApuntesProgramacion/02-variables-tipos-operadores/08-math-random-aleatorios) · **Siguiente:** [10 · Repaso interactivo](/ApuntesProgramacion/02-variables-tipos-operadores/10-repaso-interactivo)
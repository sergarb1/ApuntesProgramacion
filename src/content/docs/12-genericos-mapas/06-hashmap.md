...
> 🗺️ **Estás en:** 🗺️ **U12 · Genéricos y Mapas** → 06 · HashMap: la guía telefónica
...
> 💡 **Consejo:** para recorrer, declara el mapa como `Map<String, Integer>` y usa `Map.Entry<K, V>` en el bucle: así, si mañana cambias a `TreeMap`, no tocas nada. Programa contra la interfaz, como en la U11.
...
La **C**. En "banana", "b-a-n-a-n-a": b=1, a=3, n=2. Así que `frec.get('a')` = 3 y `frec.get('n')` = 2 → **3 2**. El truco de `getOrDefault(c, 0) + 1` es el héroe: si la letra no estaba, devuelve 0 y empieza en 1; si estaba, suma uno al contador. Este patrón es EL patrón de los mapas: lo verás en la U123 con ficheros y en media carrera de programación.
...
📚 [Volver al índice de la unidad](/ApuntesProgramacion/12-genericos-mapas) · **Anterior:** [05 · Wildcards: `? extends T` y `? super T`](/ApuntesProgramacion/12-genericos-mapas/05-wildcards) · **Siguiente:** [07 · TreeMap, LinkedHashMap y otros mapas](/ApuntesProgramacion/12-genericos-mapas/07-treemap-otros-mapas)
...
> 🗺️ **Estás en:** 🅿️ **U05 · Arrays** → 06 · Aplicaciones de los arrays
...
> ⚠️ **Advertencia:** con `String` nunca compares con `==`. Usa `.equals()`. Lo viste en la U03 y en los arrays es igual de obligatorio: `fruta == "pera"` compara referencias, no texto.

### El más famoso de todos: `args`

El array de `String` más usado del curso lo llevas escribiendo desde la U02: `main(String[] args)`. En el punto 5 viste que contiene los argumentos de la línea de comandos. Pues eso: un `String[]` de verdad, como el de tu clase.
...
> 💡 **Consejo:** si necesitas "cambiar una letra", con `String` no puedes. Una opción es pasarla a `char[]`, modificarla y volver a construir la `String`. En la U123 (ficheros y regex) esta idea te va a salir varias veces.

---

## 🧍 Arrays de objetos: el parking de personas

Aquí es donde el parking brilla de verdad. Los arrays no solo guardan primitivos o `String`: guardan **objetos** de cualquier clase. Como aún no has creado tus propias clases (eso es la U07), usa una sencilla para el ejemplo:
...
📚 [Volver al índice de la unidad](/ApuntesProgramacion/05-arrays) · **Anterior:** [05 · Arrays y métodos](/ApuntesProgramacion/05-arrays/05-arrays-metodos) · **Siguiente:** [07 · Be the Code: el parking se gestiona](/ApuntesProgramacion/05-arrays/07-be-the-code)
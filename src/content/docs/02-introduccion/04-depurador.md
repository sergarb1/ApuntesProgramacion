...
El bucle `for` lo verás a fondo en la U04, pero lo puedes intuir: repite la línea de dentro mientras `i` sea menor que 10, con `i` valiendo 0, 1, 2... La línea clave es `sospechoso += i` (que es lo mismo que `sospechoso = sospechoso + i`).
